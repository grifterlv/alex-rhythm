...
import {focusScenes,type FocusScene} from '@/lib/focus';
...
export default function ActivityScene({scene,moving}:{scene:FocusScene,moving:boolean}){
 ...
 return <div className="activity-scene"><div ref={host} className="scene-canvas-host" role="img" aria-label={focusScenes[scene]+' 3D 活动场景'}/>{status==='loading'&&<div className="scene-loading" role="status"><Loader2 className="spin"/>正在打开小场景…</div>}{status==='error'&&<div className="scene-loading scene-error" role="status"><Box size={34}/><strong>{focusScenes[scene]}</strong><p>这个设备暂时没有显示出 3D 场景。任务和计时仍可继续。</p><Button variant="outline" onClick={()=>setAttempt(n=>n+1)}>重试场景</Button></div>}</div>
}
