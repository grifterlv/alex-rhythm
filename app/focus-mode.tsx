...
import {Dialog,DialogContent,DialogDescription,DialogTitle} from './localized-dialog';
import {focusElapsed,type FocusScene} from '@/lib/focus';
 ...
 return <Dialog open={p.open} onOpenChange={v=>!v&&p.onClose()}><DialogContent className="focus-space" showCloseButton={false} onOpenAutoFocus={e=>{e.preventDefault();heading.current?.focus()}}>
  ...
  <div className="focus-space-body">
   ...
   <section className="focus-task-panel">
    ...
    <div className="focus-total"><span>{tr("这一项累计投入")}</span><div role="timer" aria-live="off" aria-label={tr("累计投入 ")+elapsed(total)}>{elapsed(total)}</div></div>
    <p className="focus-session-detail">{p.running&&p.startedAt?tr("这次开始于 {0}", [hm(clock(p.startedAt).min)]):p.done?tr("实际记录已保留。"):paused?tr("已保存的时间会接着累计。"):tr("点击开始后，才会记录实际用时。")}{p.block&&tr(" · 计划 {0}–{1}", [hm(p.block.start), hm(p.block.end)])}</p>
    {p.error&&<div className="focus-error" role="alert">{tr(p.error)}</div>}
    {!p.online&&<p className="focus-offline-note">{tr("正在显示上次同步的记录。恢复连接后再保存；计时不会因切换页面自动暂停。")}</p>}
    {p.running&&<TimerCheck due={p.reviewDue} busy={p.busy||p.loading} onReview={p.onReview}/>}
    <div className="focus-space-actions">{p.running?<><Button className="focus-primary-action" disabled={p.busy||p.loading} onClick={p.onPause}><Pause/>{tr("暂停并保存")}</Button>{p.canFinish&&<Button variant="outline" disabled={p.busy||p.loading} onClick={p.onFinish}><Check/>{tr("完成这一项")}</Button>}</>:p.done?<><Button className="focus-primary-action" disabled={p.busy||p.loading} onClick={p.next?p.onNext:p.onChoose}>{p.next?tr("看看下一项"):tr("回到日程")}<ArrowRight/></Button></>:<><Button className="focus-primary-action" disabled={p.busy||p.loading} onClick={p.canStart?p.onStart:p.onChoose}>{p.canStart?<Play/>:<Plus/>}{p.canStart?(paused?tr("继续这一项"):tr("开始专注")):tr("选择今天的任务")}</Button>{p.canFinish&&<Button variant="outline" disabled={p.busy||p.loading} onClick={p.onFinish}><Check/>{tr("已做完")}</Button>}</>}</div>
    <div className="focus-space-secondary">{p.done&&p.canUndo&&<Button variant="ghost" disabled={p.busy} onClick={p.onUndo}>{tr("撤销完成")}</Button>}<Button variant="ghost" disabled={p.busy||p.loading} onClick={p.onRecord}><Plus size={16}/>{tr("补记用时")}</Button><Button variant="ghost" onClick={p.onChoose} disabled={p.busy||p.loading}>{tr("选择其他任务")}</Button></div>
